...
namespace MiniFrames
{
    public partial class EditPetForm : Form
    {
        ...
        private readonly Pet _editingPet;
        ...
        public EditPetForm(Pet editingPet)
        {
            _editingPet = editingPet;
            InitializeComponent();
            InitializeForm();
        }
        ...
        private void InitializeForm()
        {
            ...
            {
                Text = $"Edit Pet: {_editingPet.Name}",
            ...
            (Panel w, TextBox tb) MakeTextBox(int y, int h, string value, bool multi = false)
            {
            ...
            (Panel w, ComboBox cb) MakeComboBox(int y, string[] items, string selected)
            {
                ...
                cb.SelectedIndex = cb.Items.IndexOf(selected) >= 0
                    ? cb.Items.IndexOf(selected) : 0;
            ...
            var (wName, _tb1) = MakeTextBox(118, 24, _editingPet.Name);
            tbPetName = _tb1;
            scrollPanel.Controls.Add(wName);

            scrollPanel.Controls.Add(MakeLabel("Species *", 158));
            var (wSpecies, _cb1) = MakeComboBox(180,
                new[] { "Dog", "Cat", "Bird", "Other" },
                _editingPet.Species);
            cbSpecies = _cb1;
            scrollPanel.Controls.Add(wSpecies);

            scrollPanel.Controls.Add(MakeLabel("Breed *", 220));
            string[] initialBreeds = _editingPet.Species switch
            {
                "Cat" => CatBreeds,
                "Bird" => BirdBreeds,
                "Other" => OtherBreeds,
            ...
            var (wBreed, _cb2) = MakeComboBox(242, initialBreeds, _editingPet.Breed);
            cbBreed = _cb2;
            ...
            var (wColor, _tb2) = MakeTextBox(304, 24, _editingPet.Color);
            tbColor = _tb2;
            ...
            // Pre-load existing photo if available
            if (!string.IsNullOrEmpty(_editingPet.PhotoPath) &&
                System.IO.File.Exists(_editingPet.PhotoPath))
                pbPhoto.Image = Image.FromFile(_editingPet.PhotoPath);
            ...
            scrollPanel.Paint += (s, e) =>
            {
                ...
                using (var path = RoundedPath(new Rectangle(64, 660, 120, 42), 20))
                    g.FillPath(brush, path);
                using (var brush = new SolidBrush(Color.FromArgb(50, 37, 99, 235)))
        ...
        private void OnUpdatePet(object? sender, EventArgs e)
        {
            if (string.IsNullOrWhiteSpace(tbPetName.Text))
            {
                MessageBox.Show("Please enter the pet's name.", "Missing Field",
            ...
            {
                PetID = _editingPet.PetID,
                Name = tbPetName.Text.Trim(),
            ...
            // await controller.UpdatePet(updatedPet.PetID, updatedPet);
